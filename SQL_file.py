...
class SQL():
    ...
    def __init__(self):
        self.PostgreSQLConnection = psycopg2.connect(database="bike1", user="postgres", password="252012",
    ...
    def get_order_unsold(self):
        ...
        try:
            self.pgsql_cursor.execute("""
            select o_ono, o_sno, o_up_date, o_price from o_order where 
            o_if_sold = false and o_if_approved = true;
            """)
            orders = self.pgsql_cursor.fetchall()
        except:
            self.pgsql_cursor.execute("abort")
    ...
    def get_order_unsold_by_tag(self, bt_tag):
        ...
        for each_order in order_list:
            try:
                self.pgsql_cursor.execute("""
                select o_ono, o_sno, o_up_date, o_price from o_order where o_if_sold = false
                and o_if_approved = true and o_ono =  '{o_ono}';
                """.format(o_ono=each_order))
                order = self.pgsql_cursor.fetchall()
                if order:
                    order_detail.append(order[0])
            except:
                self.pgsql_cursor.execute("abort")
                return ''
        # return order_detail
    ...
    def get_order_unsold_by_sno(self, o_sno):
        ...
        try:
            self.pgsql_cursor.execute("""
            select o_ono, o_sno, o_up_date, o_price from o_order where o_if_approved = true and o_sno =  '{o_sno}';
            """.format(o_sno=o_sno))
